...
def cnn(images):
	...
	# Conv layer 1
	conv1_w = tf.Variable(
		tf.truncated_normal([9, 9, NUM_CHANNELS, 100], stddev=np.sqrt(2) / np.sqrt(NUM_CHANNELS * 9 * 9)), name='weights_1')
	...
	# Conv layer 2
	conv2_w = tf.Variable(tf.truncated_normal([5, 5, 100, 200], stddev=np.sqrt(2) / np.sqrt(100 * 5 * 5)), name='weights_2')
	...
	logits = tf.matmul(full_layer_drop, softmax_w) + softmax_b
