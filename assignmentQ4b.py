...
num_neuron1 = 60 #num from previous qn
num_neuron2 = 20
num_neuron3 = 20
...
W1 = tf.Variable(tf.truncated_normal([NUM_FEATURES, num_neuron1], stddev=1.0 / np.sqrt(NUM_FEATURES), dtype=tf.float32), name='weights1')
B1 = tf.Variable(tf.zeros([num_neuron1]), dtype=tf.float32, name='biases1')

W2 = tf.Variable(tf.truncated_normal([num_neuron1, num_neuron2], stddev=1.0 / np.sqrt(num_neuron1), dtype=tf.float32), name='weights2')
B2 = tf.Variable(tf.zeros([num_neuron2]), dtype=tf.float32, name='biases2')

W3 = tf.Variable(tf.truncated_normal([num_neuron2, num_neuron3], stddev=1.0 / np.sqrt(num_neuron2), dtype=tf.float32), name='weights3')
B3 = tf.Variable(tf.zeros([num_neuron3]), dtype=tf.float32, name='biases3')

W4 = tf.Variable(tf.truncated_normal([num_neuron3, 1], stddev=1.0 / np.sqrt(num_neuron3), dtype=tf.float32), name='weights4')
B4 = tf.Variable(tf.zeros([1]), dtype=tf.float32, name='biases4')
# L1 = tf.matmul(x,W1) + B1
# H1 = tf.nn.relu(L1)
H1 = tf.nn.relu(tf.matmul(x, W1) + B1)
H2 = tf.nn.relu(tf.matmul(H1, W2) + B2)
H3 = tf.nn.relu(tf.matmul(H2, W3) + B3)
y_None = tf.matmul(H3,W4) + B4
...
y_drop = tf.matmul(H3_dropout,W4) + B4
